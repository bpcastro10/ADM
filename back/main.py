...
from services.ai_evaluator import evaluate_code, evaluate_written_test
...
async def evaluate_with_file_upload(
    ...
    """
    Evalúa código cargado desde archivo o pegado como texto.
    Acepta archivos .txt, .py, .js, .java, etc. y también .zip (proyectos multi-archivo).
    """
    if not file and not code_text:
        raise HTTPException(
            status_code=400,
            detail="Debe proporcionar un archivo (file) o texto de código (code_text).",
        )
    ...
    if not code or not code.strip():
        raise HTTPException(status_code=400, detail="El código está vacío.")
