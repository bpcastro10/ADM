...
export function useCodeEvaluation({ candidateName, onClearCombined }) {
  ...
  const validateNotebook = () => {
    if (!candidateName.trim()) return 'Ingrese el nombre del candidato'
    if (!notebookFile) return 'Suba el archivo de notebook (.ipynb)'
  ...
  const _downloadResultPdf = (res, prefix) => {
    fetch(`${API_BASE}/generate-pdf`, {
      ...
      .then((blob) => {
        downloadBlob(blob, `${prefix}_${res.candidate_name.replace(/\s/g, '_')}_${new Date().toISOString().slice(0, 10)}.pdf`)
      })
  ...
  const evaluateWritten = async (downloadPdf = false) => {
    ...
    setError(null); setResult(null); onClearCombined?.(); setLoading(true)
    ...
    try {
      ...
      setResult(data)
      if (downloadPdf) _downloadResultPdf(data, 'evaluacion_escrita')
    } catch (e) {
  ...
  const evaluateNotebook = async (downloadPdf = false) => {
    ...
    try {
      const form = new FormData()
      form.append('candidate_name', candidateName.trim())
      form.append('file', notebookFile, notebookFile.name)
      ...
      const data = await res.json()
      setResult(data)
      if (downloadPdf) _downloadResultPdf(data, 'evaluacion_notebook')
    } catch (e) {
  ...
  const handleDownloadPdf = () => {
    if (codeEvalSubTab === 'written') {
      result && !result.message ? _downloadResultPdf(result, 'evaluacion_escrita') : evaluateWritten(true)
      return
    }
    if (codeEvalSubTab === 'notebook') {
      result && !result.message ? _downloadResultPdf(result, 'evaluacion_notebook') : evaluateNotebook(true)
      return
    }
    result && !result.message ? _downloadResultPdf(result, 'evaluacion') : evaluate(true)
  }
